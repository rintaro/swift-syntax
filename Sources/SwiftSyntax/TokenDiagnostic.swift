...
public struct TokenDiagnostic: Hashable, Sendable {
  ...
  ///
  /// Declared before ``byteOffset`` deliberately: the other order costs 0.25% of
  /// a parse, since a lexeme carries a `TokenDiagnostic?` per token and the
  /// optional is cheaper to form and read when the kind comes first.
