...
public struct SyntaxChildren: BidirectionalCollection, Sendable {
  ...
  private static func _shouldTraverse(_ dataRef: SyntaxDataReference?, viewMode: SyntaxTreeViewMode) -> Bool {
  ...
  public func index(after index: SyntaxChildrenIndex) -> SyntaxChildrenIndex {
    ...
    return Index(
      value: layoutBuffer[(index.value + 1)...]
        .firstIndex(where: { Self._shouldTraverse($0, viewMode: viewMode) }) ?? layoutBuffer.endIndex
    )
  }
  ...
  public func index(before index: SyntaxChildrenIndex) -> SyntaxChildrenIndex {
    ...
    return Index(
      value: layoutBuffer[..<index.value]
        .lastIndex(where: { Self._shouldTraverse($0, viewMode: viewMode) })!
    )
  }
  ...
  internal init(_ node: Syntax, viewMode: SyntaxTreeViewMode) {
    ...
    self.startIndex = Index(
      value: layoutBuffer.firstIndex(where: { Self._shouldTraverse($0, viewMode: viewMode) }) ?? layoutBuffer.endIndex
    )
  }
