...
| **Static `recoveryPrecedence` and `allowAtStartOfLine` on `TokenSpecSet`** | **+0.04%**, and **+0.09% / +0.12%** with the hand-written sets converted too | The point was to stop `canRecoverTo(anyIn:)` building `allCases` at all: a protocol requirement for the two facts it reduces the array to, answered by a `static let` in each of the 62 generated sets, with default implementations for the 21 hand-written ones. It measures worse, and the specialized code says why. `canRecoverTo<DeclarationStart>` on the branch already contains **no allocation** — the optimizer turns a concrete enum's `allCases` into a constant array and folds the reduction, leaving 3 `swift_bridgeObjectRelease` calls on an immortal object — where the `static let` version is 104 instructions smaller but calls `swift_once` twice per invocation to reach the two globals. **A `static let` is not free where the computation it replaces was already constant-folded**, and the hoist in `a196f3daa` had therefore already collected everything there was: what it saved was doing that fold's release traffic three times rather than once. Kept nowhere; `EitherTokenSpecSet` cannot hold a `static let` at all, being generic. |
...
| **Widen `Layout.byteLength` to an `Int`**, so a subtree's length is not capped at 32 bits | **+11.8% arena memory**, instructions flat | The cap is not this field's to lift. `AbsoluteSyntaxInfo.offset` is `UInt32` on `main` as well, and `AbsoluteSyntaxInfo.swift` requires `raw.totalLength.utf8Length <= UInt32.max` and `raw.totalNodes <= UInt32.max` when a raw tree becomes a `Syntax`, so a tree over 4 GiB is already refused with a message. Widening leaves that limit exactly where it was and costs 8 bytes of stride on every layout node — `Layout` goes from 15/16 to 19/24, because an `Int` is 8-aligned — which is 10.14× the source to **11.34×** over the corpus, 97.8 MB to 109.2 MB of requested bytes. Instructions are flat on both inputs and on all three consumer paths, and **the field's position makes no difference**: first or after the two counts, the stride is 24 either way, and the builder writes the fields one at a time regardless. Recorded at the declaration, since that is where the next reader will have the same concern. |
